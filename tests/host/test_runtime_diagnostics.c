...
int main(void)
{
    ...
    const ev_actor_id_t boot_last_target = ev_test_last_route_target_for_event(EV_BOOT_COMPLETED);
    ...
    assert(ev_actor_runtime_init(&bh1750_runtime, ACT_BH1750, &bh1750_mailbox, ok_handler, &bh1750_calls) == EV_OK);
    ...
    registry_stats = ev_actor_registry_stats(&registry);
    assert(registry_stats->delivery_attempted == boot_routes);
    assert(registry_stats->delivery_succeeded == boot_routes);
    assert(registry_stats->delivery_failed == 0U);
    assert(registry_stats->delivery_target_missing == 0U);
    assert(registry_stats->last_target_actor == boot_last_target);
    assert(registry_stats->last_result == EV_OK);
    ...
    assert(ev_actor_runtime_stats(&bh1750_runtime)->enqueued == 1U);
    ...
    assert(report.failed_count == boot_routes - 1U);
    ...
    assert(registry_stats->delivery_failed == boot_routes - 1U);
    assert(registry_stats->delivery_target_missing == boot_routes - 1U);
    assert(registry_stats->last_target_actor == boot_last_target);
