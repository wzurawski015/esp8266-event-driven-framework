...
def redact_tree(root: Path, *, apply: bool) -> tuple[int, int]:
    files_changed = 0
    for path in iter_text_files(root):
        if redact_file(path, apply=apply):
            files_changed += 1
...
def self_test() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        manifest = {"evidence_kind": "wemos_one_shot_bundle", "operator_intent": {"ok": True}, "stages": [{"log": "serial.raw.log", "sha256": old_sha}, {"log": "parsed.json", "sha256": sha256(run / "parsed.json")}], "wemos_smoke": {"serial_sha256": old_sha, "serial_raw_sha256": old_sha, "serial_normalized_sha256": old_sha}, "target_timing": {"sha256": sha256(run / "target_timing.json")}}
        ...
        assert repairs >= 1
        for path in iter_text_files(root):
            assert "lab-ssid-value" not in path.read_text(encoding="utf-8", errors="ignore")
...
def main() -> int:
    ...
    if args.self_test:
        self_test()
        return 0
    files, repairs = redact_tree(args.root, apply=args.apply)
