...
static void ev_hil_log_onewire_pin_map(const ev_esp8266_onewire_hil_config_t *cfg)
{
    ESP_LOGI(EV_HIL_ONEWIRE_TAG,
             "EV_HIL_ONEWIRE_PIN_MAP board=%s dq_gpio=%d pullup=external required=1 source=bsp/atnel_air_esp_motherboard/pins.def wifi=off",
             (cfg != NULL && cfg->board_tag != NULL) ? cfg->board_tag : "unknown",
             (cfg != NULL) ? cfg->dq_gpio : -1);
    ESP_LOGI(EV_HIL_ONEWIRE_TAG,
             "EV_HIL_ONEWIRE_WIFI_TIMING status=ENVIRONMENT_BLOCKED reason=wifi_on_timing_transcript_not_attached");
}
...
static void ev_hil_irq_flood_log_stack(const char *task_name)
{
    ...
    if (s_ev_hil_irq_flood_task_handle == NULL) {
        ESP_LOGW(EV_HIL_ONEWIRE_TAG, "EV_HIL_STACK task=%s status=not_available", (task_name != NULL) ? task_name : "irq-flood");
        return;
    }
    ESP_LOGI(EV_HIL_ONEWIRE_TAG,
...
#else
    (void)task_name;
    ESP_LOGW(EV_HIL_ONEWIRE_TAG, "EV_HIL_STACK task=irq-flood status=not_available");
#endif
}
...
static void ev_hil_log_onewire_diag(const char *stage)
{
    ...
    if (ev_esp8266_onewire_get_diag(&diag) == EV_OK) {
        ESP_LOGI(EV_HIL_ONEWIRE_TAG,
                 "onewire-diag:%s ops=%u crit=%u reset_crit=%u bit_crit=%u max_us=%u reset_max_us=%u bit_max_us=%u budget_violations=%u reset_low_us=%u bus_errors=%u busy=%u dq_high=%u",
                 stage,
                                               ...
                                               ev_hil_suite_result_t *result)
{
    ...
    for (i = 0U; i < cfg->ds18b20_read_iterations; ++i) {
        ...
        ESP_LOGI(EV_HIL_ONEWIRE_TAG,
                 "EV_HIL_ONEWIRE_DS18B20_SCRATCHPAD_CRC name=%s iteration=%u status=PASS",
                 name,
                 (unsigned)i);
    ...
    } else if (after_ow.bus_errors != before_ow.bus_errors) {
        ev_hil_fail(result, name, "OneWire bus error observed during DS18B20 flood test");
    } else if (after_ow.busy || !after_ow.dq_high) {
        ev_hil_fail(result, name, "OneWire DQ line was not released after DS18B20 transaction");
    } else {
        ESP_LOGI(EV_HIL_ONEWIRE_TAG,
                 "EV_HIL_ONEWIRE_TIMING reset_low_us=%u reset_high_us=410 presence_low_us=unknown slot_min_us=60 slot_max_us=120 recovery_min_us=1 wifi=off",
