...
int main(void)
{
    ...
    size_t before_turns;
    size_t injected = 0U;
    const size_t total_target = 100U;
    ...
    drive_app_until_quiescent(&app, &fake_irq, 64U);
    assert(app.app_actor.time_valid);

    while ((injected < total_target) && (fake_irq_port_push(&fake_irq, &sample) == EV_OK)) {
        sample.timestamp_us += 1U;
        ++injected;
    }
    assert(injected > 0U);
    ...
    assert((fake_irq.count > 0U) || (ev_demo_app_pending(&app) > 0U) || (injected < total_target));

    while (((fake_irq.count > 0U) || (ev_demo_app_pending(&app) > 0U) || (injected < total_target)) &&
           (iterations < 128U)) {
        ev_result_t rc;
    ...
    assert(iterations < 128U);
    assert(injected == total_target);
    assert(fake_irq.count == 0U);
    assert(ev_demo_app_pending(&app) == 0U);
    assert(app.app_actor.time_valid);

    printf("Starvation test passed: execution bounded correctly.\n");
    return 0;
}
