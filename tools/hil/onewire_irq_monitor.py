#!/usr/bin/env python3
"""Serial monitor gate for the ESP8266 OneWire IRQ HIL firmware."""
...
SUCCESS_RE = re.compile(r"EV_HIL_RESULT PASS failures=0 skipped=0|onewire IRQ HIL completed successfully")
FAIL_RE = re.compile(r"EV_HIL_CASE .* FAIL|EV_HIL_RESULT FAIL|onewire IRQ HIL failed")
...
def main(argv: list[str]) -> int:
    if len(argv) == 2 and argv[1] == "--self-test":
        return self_test()
    if len(argv) != 4:
        print("usage: onewire_irq_monitor.py <serial-port> <baud> <timeout-s>", file=sys.stderr)
        return 2
    ...
    print("error: timed out waiting for OneWire IRQ HIL success summary", file=sys.stderr)
    return 1
