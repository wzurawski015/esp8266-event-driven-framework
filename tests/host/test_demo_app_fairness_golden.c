...
int main(void)
{
    fake_clock_t clock = {0}; ev_clock_port_t clock_port = {&clock, fake_mono_now_us, fake_wall_now_us, fake_delay_ms};
    fake_log_port_t fake_log; ev_log_port_t log_port; ev_demo_app_t app; ev_demo_app_config_t cfg;
    const ev_system_pump_stats_t *pump_stats;
    fake_log_port_init(&fake_log); fake_log_port_bind(&log_port, &fake_log);
    memset(&cfg, 0, sizeof(cfg));
    cfg.app_tag = "fairness_golden"; cfg.board_name = "host_no_hw"; cfg.tick_period_ms = 1000U;
    cfg.clock_port = &clock_port; cfg.log_port = &log_port; cfg.board_profile = ev_demo_app_default_board_profile();
